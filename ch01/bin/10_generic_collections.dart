...
void main() {
  ...
  String text = stringBox.getTiem();
  print('text : $text');

  Box intBox = Box<int>(100);
  int num = intBox.getTiem();
  print('num : $num');
  ...
  // 리스트 반복문
  for(String fruit in fruits){ // String → var 대체 가능(유연성 있게)
    print('과일 : $fruit');
  }
  ...
  // 변환
  var doubledList = numbers.map((n) => n * 2).toList(); // entity를 DTO로 변환할 때 spring에서 쓴다
  ...
  // 원소 추가
  colors.add('orange');
  colors.add('red'); // red가 이미 있어서 중복 안됨
  print('colors : $colors');
